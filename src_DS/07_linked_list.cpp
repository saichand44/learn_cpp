...
void reverseList(node* currNode, node* &head)
{
    ...
    reverseList(currNode->next, head);
    node* tempNode = currNode->next;      // store info about the next node
    tempNode->next = currNode;            // point next node to backward / current node
    currNode->next = NULL;                // handle the condition to assign NULL at end of list
}
